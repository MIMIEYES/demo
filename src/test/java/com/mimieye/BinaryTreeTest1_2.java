 ...
 */
public class BinaryTreeTest1_2<T extends Comparable> {
    ...
    public static void main(String[] args) {
        BinaryTreeTest1_2<Integer> bt = new BinaryTreeTest1_2<>();
        bt.putValue(5);
        bt.putValue(6);
        bt.putValue(16);
        bt.putValue(26);
        ...
        for(int i=1;i<=size;i++){
            ...
            if(i == 1){
                ...
                if(i != size){
                    ...
                    if(lineSize > 2)
                        splitLineNodePairSpaceSize = (lineOutterMiddleSpaceSize + 2 - lineSize - lineSize/2 * lineNodeMiddleSpaceSize)/(lineSize/2 -1);
                    if(getDeepNode(node, i+1, 1) != null)
                        System.out.print(StringUtils.leftPad("/", lineFirstSpaceSize));
                    else
                        System.out.print(StringUtils.leftPad(" ", lineFirstSpaceSize));
                    for(int k=2;k<=lineSize;){
                        System.out.print(StringUtils.leftPad("", lineNodeMiddleSpaceSize));
                        if(k == lineSize){
                            if(getDeepNode(node, i+1, k) != null){
    ...
    private Node getDeepNode(Node node, int y, int x){
    ...
    private String getDeepNodeValue(Node node, int y, int x){
        return getNodeValue(getDeepNode(node, y, x));
    }

    private int power2sum(int n){
        if(n == -1) return 1;
    ...
    private int nodeMiddleSpace(int n){
        if(n == 2)return 1;
        if(n == 1)return 0;
        return power2sum(n-3) * 3;
    }

    private int keyFirstSpace(int n){
        if(n == 1)return 0;
        return pow(2,n -2) * 3;
    }

    private int lineOutterMiddleSpace(int prevOutterMsize, int n){
